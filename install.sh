#!/usr/bin/env bash
# iptv-spider-pro 一键安装引导（PVE 侧，单仓库自包含）
#
# 本仓库同时包含 PVE 编排脚本与应用本体（app/），全新安装只需这一个仓库，
# 不依赖其它 GitHub 仓库：默认把 app/ 作为本地发行包推入新 CT；GitHub 兜底
# 也只指向本仓库 Release。
set -euo pipefail

VERSION=${IPTV_SPIDER_PRO_VERSION:-v1.2.0}
REPO=driftbottle61/iptv-spider-pro
...
在 Proxmox VE Shell 以 root 运行：
  bash <(curl -fsSL https://raw.githubusercontent.com/driftbottle61/iptv-spider-pro/v1.2.0/install.sh)
...
tmp=$(mktemp -d /tmp/iptv-spider-pro.XXXXXX)
trap 'rm -rf "$tmp"' EXIT
archive="${tmp}/${VERSION}.tar.gz"
url="https://github.com/${REPO}/archive/refs/tags/${VERSION}.tar.gz"
echo "下载 iptv-spider-pro ${VERSION} ..."
curl -fL --retry 3 --retry-delay 2 -o "$archive" "$url"
tar -xzf "$archive" -C "$tmp"
root_dir="$tmp/iptv-spider-pro-${VERSION#v}"
